...
$execute if score @s defense.element_random matches 91..100 as @n[tag=defense-monster,tag=defense-target] at @s run function core:defense/towers/element/wind/blow_back {"damage":"$(wind_damage)","ignite_time":"$(ignite_time)"}
$execute if score @s defense.element_random matches 1..70 as @n[tag=defense-monster,tag=defense-target] at @s run function core:defense/towers/element/earth/stab {"damage":"$(earth_damage)"}