...
$execute as @n[tag=defense-monster,tag=defense-target] at @s run function core:defense/towers/storm/blow_back_$(type) {"damage":"$(damage)","blow_power":"$(blow_power)","freeze_power":"$(freeze_power)","freeze_time":"$(freeze_time)"}