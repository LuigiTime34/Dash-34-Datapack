...
execute if score #hit vdvcasttemp matches 0 positioned ~-0.05 ~-0.05 ~-0.05 as @e[tag=!vdvray,dx=0,sort=nearest,tag=defense-monster,tag=!defense-baby_zombie,tag=!defense-chicken,tag=!defense-silverfish,tag=!defense-endermite,tag=!defense-spider,tag=!defense-zoglin] run function core:defense/towers/bee/raycast/check_hit_entity
